...
public class Solution {
    ...
    public <T extends Item> T getProxy(Class<T> mainClass, Class<?>... interfaces) {
        ClassLoader classLoader = mainClass.getClassLoader();
        Class<?>[] newInterfaces = new Class[interfaces.length + 1];
        newInterfaces[0] = mainClass;
        for (int i = 0; i < interfaces.length; i++) {
            newInterfaces[i + 1] = interfaces[i];
        }
        return (T) Proxy.newProxyInstance(classLoader, newInterfaces, new ItemInvocationHandler());
    }
}
